...
def distribute_layers(num_layers: int, pp_rank: int, pp_world_size: int) -> List:
        ...
        """
        start_layer = pp_rank * (num_layers // pp_world_size)
        end_layer = start_layer + (num_layers // pp_world_size)
        if pp_rank == pp_world_size - 1:  # Last stage takes the remainder
            end_layer += num_layers % pp_world_size
        layers_in_current_stage = list(range(start_layer, end_layer))
