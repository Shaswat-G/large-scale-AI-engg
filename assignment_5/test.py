...
def main(pp: int,
    ...
    # Q2: Assert args
    assert number_of_layers % pp == 0, "Number of layers must be divisible by pp"
    assert global_batch_size % micro_batch_size == 0, "Global batch size must be divisible by micro batch size"
    num_micro_batches = global_batch_size // micro_batch_size
    assert num_micro_batches >= 2*pp, "Number of micro batches must be greater than or equal to 2*pp for maxium throughput"
    ...
    for _ in range(number_of_microbatches): # All forward passes
        ...
        micro_batch = micro_batch.to(device_id) if micro_batch is not None else None
        # 3. Compute the forward pass
        input_to_model = micro_batch if micro_batch is not None else input_tensor
    ...
    for pp_stage_layer in model_stage.pp_stage_layers:
        ...
        idx = pp_stage_layer.layer_idx
        reference_layer = model.layers[idx]

    # Compare gradients
    torch.testing.assert_close(pp_stage_layer.fc1.weight.grad, reference_layer.fc1.weight.grad)
    torch.testing.assert_close(pp_stage_layer.fc2.weight.grad, reference_layer.fc2.weight.grad)
    ################################################### 
    torch.cuda.synchronize()
    dist.barrier()
