...
def check(tensor, expected_shape):
    ...
    assert len(expected_shape) == len(actual_shape), (f"SHAPE GUARD: Expected shape length {len(expected_shape)} but got {len(actual_shape)} with shape {actual_shape}.") 
    ...
    for idx, (actual_dim, expected_dim) in enumerate(zip(actual_shape, expected_shape)):
        ...
        if expected_dim <= 0:
            continue
        assert actual_dim == expected_dim, (f"SHAPE GUARD: At dimension {idx}: expected {expected_dim}, but got {actual_dim} (actual shape: {actual_shape}).")